...
from gfcc_dam_common_utility_configuration.gfcc_dam_dag_utility import log_msg, sqlexecute_bq, readfrombucket, bq_to_csv, throw_exception, get_schema_dict, read_json
            ...
            for _, row in df.iterrows():
                if int(row["dc_id"]) == dc_id:
                    ...
                    if valid_run:
                        ...
                        cols_str = ''
                    dc_audit_table(project_id, table_prefix, dc_id, user_email, status, load_type, tgt_ds, tgt_tbl, tgt_tbl_name, file_name, failed_rsn,
...
def create_tbl(project_id, table_prefix, load_type, tgt_ds, tgt_tbl, src_ds, src_db, src_tbl, col_list, where_cond, csv_path, dttm, limit_val, file_name):
    ...
    try:
        ...
        if load_type.upper() == "SNAPSHOT":
            if all(v != "None" for v in [tgt_ds, tgt_tbl, file_name, csv_path]):
                return snapshot_table(project_id, table_prefix, tgt_ds, tgt_tbl, file_name, csv_path, schema_changed, csv_col_chk_str)
            else:
            ...
            if all(v != "None" for v in [src_db, src_ds, src_tbl, col_list, csv_path]):
                ...
                bq_to_csv_client(project_id, src_db, src_ds, src_tbl, col_list, where_str, limit_str, csv_path)
