...
def snapshot(client: bigquery.Client, project_id: str, table_col_list: str, bq_src_db: str, bq_src_dataset: str, bq_src_table: str, 
             csv_path:str, file_name: str, bq_target_db:str, bq_target_dataset:str, bq_target_table:str, where_str:str=None,limit_str:str=None):
	try:
		sqlstr = "select {} from `{}.{}.{}` {} {}".format(table_col_list,bq_src_db,bq_src_dataset,bq_src_table,where_str,limit_str)
		...
		ddl_temp_str = "{} `{}.{}.{}` as ({});".format(ddl_create_str,bq_target_db,bq_target_dataset,temp_tbl_nm,sqlstr)
		sqlexecute_bq(client, ddl_temp_str)
		ddl_str = "create table if not exists "
		ddl_tgt_str = "{} `{}.{}.{}` as ({});".format(ddl_str,bq_target_db,bq_target_dataset,bq_target_table,sqlstr)
